...
export const verificarToken = async (req, res) => {
  ...
  try {
    ...
    const [clienteRows] = await pool.query(
      "SELECT idCliente, rol FROM cliente WHERE correo = ? OR telefono = ? LIMIT 1 FOR UPDATE",
      [correo, telefono]
    );
    ...
    // res.cookie('session_token', token, { httpOnly: true, secure: true, maxAge: 8*3600*1000 });
